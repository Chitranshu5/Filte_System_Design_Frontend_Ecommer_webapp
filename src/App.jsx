...
const Products = ({ selectedCategory }) => {
  ...
  useEffect(() => {
   ...
   // Convert selectedCategory to lowercase if it has a capital letter
const normalizedCategory = selectedCategory.charAt(0).toUpperCase() === selectedCategory.charAt(0)
? selectedCategory.toLowerCase()
: selectedCategory;

const url = normalizedCategory === "all"
? "https://anjali-m8dj.onrender.com/getProducts"
: `https://anjali-m8dj.onrender.com/getProductcat?category=${encodeURIComponent(normalizedCategory)}`;

      const response = await fetch(url);
      const result = await response.json();
      console.log(result); // Log the full result to check its structure
      console.log(selectedCategory)
      setProducts(result.data||[]); // Set the products to the "data" array
    };
    fetchData();
  }, [selectedCategory]);
